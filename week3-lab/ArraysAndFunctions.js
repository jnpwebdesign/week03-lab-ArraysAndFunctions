...
function reverse(testValue){
    ...
    } else if (typeof testValue === "array"){
        let oppositeElements = testValue.map(function(elementOfArray) {
            let stringElementOfArray = elementOfArray.toString()
            let splitString = stringElementOfArray.split("");
            let reverseArray = splitString.reverse();
            let reversedString = reverseArray.join("");
            return reversedString;
        });
...
/* let grossInvoiced;
let profitMargin;
function howMuchBonus(grossInvoiced, profitMargin){
    for(grossInvoiced >= 1000000){
        profitMargin = .4(grossInvoiced - 1000000);

    }
    for(grossInvoiced < 1000000 && >= 500000)
